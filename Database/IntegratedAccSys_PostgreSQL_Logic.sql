...
-- Returns a single result row (IsValid, Token, UserCode) via REFCURSOR.
-- For the resultset we use a function variant below for app convenience.
...
-- sp_Login_Result: Function variant returning IsValid/Token/UserCode as a row
-- (Npgsql + ADO.NET callers use this to read the same way as a SP result set)
CREATE OR REPLACE FUNCTION public.sp_Login_Result(
    p_user_id        VARCHAR(15),
    p_password       VARCHAR(256),
    p_computer_name  VARCHAR(100) DEFAULT NULL,
    p_ip_address     VARCHAR(50)  DEFAULT NULL)
RETURNS TABLE (isvalid BOOLEAN, token UUID, usercode INT)
LANGUAGE plpgsql
AS $$
DECLARE
    v_password_hash BYTEA;
...
CREATE OR REPLACE PROCEDURE public.sp_Logout(IN p_token UUID)
LANGUAGE plpgsql
AS $$
DECLARE
    v_user_code INT;
...
-- sp_ValidateSession: Checks if session is valid and refreshes it
CREATE OR REPLACE PROCEDURE public.sp_ValidateSession(
    IN p_token UUID,
    OUT o_is_valid BOOLEAN,
    OUT o_user_code INT,
    OUT o_user_name VARCHAR(200),
