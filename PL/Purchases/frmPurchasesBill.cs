...
{
    public partial class frmPurchasesBill : Form
    {
        ...
        private void frmPurchasesBill_Load(object sender, EventArgs e)
        {
            ...
            if (txtOpType.Text == "5")
            {
                cu.ApplyPrivileges(this, 32);
            }
            if (txtOpType.Text == "6")
            {
                cu.ApplyPrivileges(this, 27);
            }
            getAllData();
        }
        ...
        private void addJournalBody()
        {
            ...
            foreach (var item in groupedData)
            {
                ...
                // الطرف المدين
                cj.addJournalBody(inventoryAccCode, currencyId, currencyRate, net, 0, note, journalNo);
                totalDebit += net;

                if (totalVAT > 0)
                {
                    cj.addJournalBody(purVatAccCode, currencyId, currencyRate, totalVAT, 0, note, journalNo);
                    totalDebit += totalVAT;
                }
                ...
                cj.addJournalBody(creditAccount, currencyId, currencyRate, 0, creditAmount, note, journalNo);
                totalCredit += creditAmount;

                if (totalDiscount > 0)
                {
                    cj.addJournalBody(purDiscAccCode, currencyId, currencyRate, 0, totalDiscount, note, journalNo);
