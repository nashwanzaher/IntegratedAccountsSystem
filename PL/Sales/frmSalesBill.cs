...
{
    public partial class frmSalesBill : Form
    {
        ...
        private void frmSalesBill_Load(object sender, EventArgs e)
        {
            ...
            if (txtOpType.Text == "4")
            {
                cu.ApplyPrivileges(this, 36);
            }
            if (txtOpType.Text == "7")
            {
                cu.ApplyPrivileges(this, 28);
            }
            getAllData();
        ...
        {
            PL.stores.frmSelectItem fst = new stores.frmSelectItem();
            if (e.KeyCode == Keys.Enter)
            {
        ...
        void addJournalBody()
        {
            ...
            if (totalCredit > 0)
            {
                cj.addJournalBody(creditAccount, currencyId, currVal, totalCredit, 0, note, jNo);
            ...
            foreach (KeyValuePair<int, double[]> entry in groupedData)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (totalDiscount > 0)
                    {
                        cj.addJournalBody(saleDiscAccCode, currencyId, currVal, totalDiscount, 0, note, jNo);
                    ...
                    if (groupCost > 0)
                    {
                        cj.addJournalBody(saleCostCode, currencyId, currVal, groupCost, 0, note, jNo);
                        totalDebitSum += groupCost;
                    }

                    if (totalPriceBeforeDiscount > 0)
                    {
                        cj.addJournalBody(saleRevenuseCode, currencyId, currVal, 0, totalPriceBeforeDiscount, note, jNo);
                        totalCreditSum += totalPriceBeforeDiscount;
                    }

                    if (groupCost > 0)
                    {
                        cj.addJournalBody(inventoryAccCode, currencyId, currVal, 0, groupCost, note, jNo);
                        totalCreditSum += groupCost;
                    }

                    if (totalVAT > 0)
                    {
                        cj.addJournalBody(saleVatAccCode, currencyId, currVal, 0, totalVAT, note, jNo);
